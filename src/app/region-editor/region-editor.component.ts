...
export class RegionEditorComponent implements OnInit {
    ...
    calculateCoverageArea(next){
        console.log(this.coords);
        let selectionArea = 0;
        let image = new Image();
        image.src = this.snapshotUrl;
        jQuery(image).load(this.snapshotUrl, null,  ()=>{
            setTimeout(()=> {
                ...
                if (this.coords !== '') {
                    ...
                    console.log('Calculating Coords on Entire Image: ' + this.monitorDetails.config.motionConfig.motionAreaPixelCount);
                }
